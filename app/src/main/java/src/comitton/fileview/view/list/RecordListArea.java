...
public class RecordListArea extends ListArea {
	private static final String TAG = "RecordListArea";
			...
			for (int index = 0; index < listSize; index++) {
				...
				if (rd.getType() == RecordItem.TYPE_FOLDER) {
					...
					if (rd.getServerName().equals("")) {
						mTitleSep[index][0] = TextFormatter.getMultiLine(res.getString(R.string.undefine) + " #" + index, tcx, mTitlePaint, 1);
					}
					else {
						mTitleSep[index][0] = TextFormatter.getMultiLine("[" + rd.getServerName() + "]", tcx, mTitlePaint, 1);
					}
					...
					if (rd.getType() == RecordItem.TYPE_IMAGE || rd.getType() == RecordItem.TYPE_IMAGEDIRECT) {
						work = rd.getImage();
					}
					else {
				...
				if (rd.getType() == RecordItem.TYPE_FOLDER) {
					...
					String uri = "";
					if (rd.getServer() == DEF.INDEX_LOCAL) {
						uri = rd.getPath();
					}
					else {
						uri = rd.getDispName();
